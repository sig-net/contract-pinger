...
async function execute({ check_signature, environment }) {
    ...
    const { chainSigContract, provider, requesterKeypair } = (0, initSolana_1.initSolanaNew)({ contractAddress, environment });
    const signArgs = (0, evmTransactions_1.getSignArgs)();
    if (check_signature) {
        const signature = await chainSigContract.sign(signArgs[0], {
            ...signArgs[1],
            remainingAccounts: [{ pubkey: requesterKeypair.publicKey, isWritable: false, isSigner: true }],
