...
async function execute({ check_signature, environment }) {
    ...
    const { chainSigContract, publicClient, walletClient } = (0, initEvm_1.initEthereum)({ contractAddress, environment });
    if (check_signature) {
        const signature = await (0, evmTransactions_1.createSignRequestAndWaitSignature)({ chainSigContract, publicClient, walletClient });
        return { signature };
    }
    else {
        const signatureRequest = await (0, evmTransactions_1.createSignRequest)({ chainSigContract, publicClient, walletClient });
